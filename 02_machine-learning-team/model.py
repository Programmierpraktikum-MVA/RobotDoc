...
model1 = tf.keras.models.load_model('model_config')
model2 = tf.keras.models.load_model('model_config2')
...
def predict(nlp_output,case):
    if(case==1):
        predict = model1.predict([pipeline(nlp_output,case)])
        
    elif(case==2):
        predict = model2.predict([pipeline(nlp_output,case)])
...
def main():
    ...
    #tracemalloc.stop()
    print(predict(process_input(sentence),1))
    print(predict(process_input(sentence),2))
    """print(tracemalloc.get_traced_memory())
